...
def param_count(matrix):
    ...
    print("\t<START><rnn_utils.py><def param_count>")
    assert isinstance(matrix, torch.nn.Module)
    total = 0
    for param in matrix.parameters():
        # print("param: matrix.parameters() 원소 하나하나씩 : {}".format(param))
        num = param.shape
        print("\tnum=param shape:{}".format(num))
        total += num.numel()            # numel():원소의 갯수
    if isinstance(matrix, TensorTrain):
        assert total == matrix.dof

    print("\t<END><rnn_utils.py><def param_count>")
    print("\t>>>RETRUN>total")
