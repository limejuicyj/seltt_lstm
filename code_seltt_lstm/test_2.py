...
if args.permute:    # each pixel is input for a time step.
    input_channels = 1      # [128,1,28,28] 데이터를 모델에 넣어줄때(output = model(data)), [batch_size,seq_len,input_size]=[128,784,1]로 들어가게됨
                            # 길이가 784짜리를 1번 넣어주는 개념
    # print("args.permuted. input_channels= {}".format(input_channels))
else:               # each row(of pixels) is input for a time step.
    input_channels = 28     # [128,1,28,28] 데이터를 모델에 넣어줄때(output = model(data)), [batch_size,seq_len,input_size]=[128,28,28]로 들어가게됨
...
sys.stdout = open(r'./results/'+ name + '.txt', 'w')  # run결과 외부에 파일로 저장
...
# print("# train_loader: 한번에 128씩, 50,000/128(batch) = 390.xx 390번 데이터를 로드해준다")
# print("# val_loader, test_loader이하동문")
