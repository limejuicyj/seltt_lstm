...
def matrix_with_random_cores(shape, tt_rank=2, mean=0., stddev=1.,
    ...
    """
    print("\t\t    <START><initializers.py><def matrix_with_random_cores>")
    ...
    for k in range(len(tt_cores)):
        print("\t\t      tt_cores[{}] : {}".format(k, tt_cores[k].shape))
    print("\t\t    <END><initializers.py><def matrix_with_random_cores>")
    print("\t\t    >>><RETURN><CALL><tensor_train.py> class TensorTrain(tt_cores)")
    return TensorTrain(tt_cores)
...
def random_matrix(shape, tt_rank=2, mean=0., stddev=1.,
    ...
    Returns:
      TensorTrain containing a TT-matrix of size
        np.prod(shape[0]) x np.prod(shape[1])
    """
    print("\t\t  <START><initializers.py><random_matrix>")
    # TODO: good distribution to init training.
    # In case the shape is immutable.
    shape = list(shape)
    # In case shape represents a vector, e.g. [None, [2, 2, 2]]
    ...
    print("\t\t  <CALL><initializers.py><def matrix_with_random_cores>")
    tt = matrix_with_random_cores(shape, tt_rank=tt_rank, stddev=core_stddev, dtype=dtype)
    print("\t\t  <CALL back> tt=matrix_with_random_cores():{}".format(tt))
    print("\t\t  <END><initializers.py><random_matrix>")
    print("\t\t  >>><RETURN> tt")
    if np.abs(mean) < 1e-8:
        return tt
    else:
...
def glorot_initializer(shape, tt_rank=2, dtype=torch.float32):
    print("\t\t<START><initializers.py><def glorot_initializer>")
    shape = list(shape)
    # In case shape represents a vector, e.g. [None, [2, 2, 2]]
    if shape[0] is None:
        shape[0] = np.ones(len(shape[1]), dtype=int)
    ...
    lamb = 2.0 / (n_in + n_out)
    print("\t\t<END><initializers.py><def glorot_initializer>")
    print("\t\t>>><RETURN><CALL> <initializers.py><random_matrix>")
