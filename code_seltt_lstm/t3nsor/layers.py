...
class TTLinear(nn.Module):
    print("\t<START><layers.py><Class TTLinear>")
    def __init__(self, in_features=None, out_features=None, bias=True, init=None, shape=None,
        ...
        print("\t<START><layers.py><Class TTLinear><def __init__>")
        ...
        if init is None:
            print("\t<CALL><initializers.py><DEF glorot_initializer>")
            init = t3.glorot_initializer(shape, tt_rank=tt_rank)
            print("\t<CALL back> init = t3.glorot_initializer() : {}".format(init))

        self.shape = shape
        print("\t<CALL><ops.py><DEF transpose>")
        self.weight_t = t3.transpose(init).to_parameter()           #ops.py에 def transpose()
        print("\t<CALL back> self.weight_t = t3.transpose(init).to_parameter():{}".format(self.weight_t))
        self.parameters = self.weight_t.parameter
        print("self.parameters= self.weight_t.parameter:{}".format(self.parameters))
        if bias:
        ...
        print("\t<END><layers.py><Class TTLinear><def __init__>")

    def forward(self, x):
        # print("\t<START><layers.py><Class TTLinear><def forward>")
        weight_t = self.weight_t
        x_t = x.transpose(0, 1)
        # print("\t<END><layers.py><Class TTLinear><def forward>")
        # print("\t>>><RETURN><CALL> t3.tt_dense_matmul()")
        if self.bias is None:
            return t3.tt_dense_matmul(weight_t, x_t).transpose(0, 1)
        else:
            return t3.tt_dense_matmul(weight_t, x_t).transpose(0, 1) + self.bias

    print("\t<END><layers.py><Class TTLinear>")