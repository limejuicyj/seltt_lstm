...
def transpose(tt_matrix):
    ...
    print("\t\t<START><ops.py><DEF transpose>")
    print("\t\tcores의 축을 아래처럼 바꾼다다")
    cores = []

    for k in range(len(tt_matrix.tt_cores)):
        print("\t\t      tt_matrix.tt_cores[{}] : {}".format(k, tt_matrix.tt_cores[k].shape))

    for core in tt_matrix.tt_cores:
        cores.append(core.transpose(1, 2))

    for k in range(len(cores)):
        print("\t\t      cores[{}] : {}".format(k, cores[k].shape))

    print("\t\t<END><ops.py><DEF transpose>")
    print("\t\t>>><RETURN><CALL> TensorTrain(cores)")
